...
class TemporaryWorkController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->hasRole(['designer','supplier','Design Checker','Designer and Design Checker']))
        // if(Auth::user()->hasRole(['designer','supplier','Design Checker','Designer and Design Checker']) || Auth::user()->hasRole('company'))
        // if((Auth::user()->hasRole(['designer','supplier','Design Checker','Designer and Design Checker']) && !auth::user()->company_id) || Auth::user()->hasRole('company'))
        {
            return redirect('designer/designer');
        }
        $user = User::with('userCompany')->find(Auth::user()->id);
    ...
    { 
        ...
        try {
            ...
            if ($temporary_work) {
                ...
                //work for pdf
                if ($request->file('photo')) {
                    $other_photo = $imagename;
                }else{
                    $other_photo = NULL;
                }
                $pdf = PDF::loadView('layouts.pdf.design_breif', ['data' => $request->all(), 'other_photo'=>$other_photo, 'name_signature'=>$image_name,'signature_type'=>$signature_type, 'image_name' => $temporaryWork->id, 'scopdesg' => $scope_of_design, 'folderattac' => $folder_attachements, 'folderattac1' =>  $folder_attachements_pdf, 'imagelinks' => $image_links, 'twc_id_no' => $twc_id_no, 'comments' => $attachcomments, "description" => $content,'image_name3' => $image_name3, 'image_name4' => $image_name4, 'image_name5' => $image_name5, 'company3' => $request->company3, 'company4' => $request->company4, 'company5' => $request->company5, 'date3'=>$request->date3, 'date4'=>$request->date4, 'date5'=>$request->date5]);
                $path = public_path('pdf');
                // if (isset($request->approval)) {
    ...
    public function permit_load(Request $request)
    {
        ...
        try {
            ...
            if ($permitdata) {
                ...
                if (isset($data)) {
                    // $count = count($data);
                    $last_number = end($data);
                    $twc_id_no = $twc_id_no . '-' . ++$last_number;}
                // } elseif (isset($data[2])) {
                //     dd('2');
    ...
    public function permit_save(Request $request)
    {
        ...
        // }
            ...
            if ($permitload) {
                ...
                //save permit image
                $pojectdata=Project::with('company')->find($request->project_id);
                $image_links = $this->permitfiles($request, $permitload->id); 
                $pdf = PDF::loadView('layouts.pdf.permit_load', ['data' => $request->all(), 'image_links' => $image_links, 'image_name' => $image_name, 'image_name1' => $image_name1, 'image_name3' => $image_name3, 'image_name4' => $image_name4, 'image_name5' => $image_name5, 'company1' => $request->company1, 'company3' => $request->company3, 'company4' => $request->company4, 'company5' => $request->company5, 'date1'=>$request->date1, 'date3'=>$request->date3, 'date4'=>$request->date4, 'date5'=>$request->date5,'file_minimum_concrete' => $file_minimum_concrete,'signature_type'=>$all_inputs['signature_type'],'signature_type1'=>$all_inputs['signature_type1'],'projectdata'=>$pojectdata]);
                $path = public_path('pdf');
                $filename = rand() . '.pdf';
    ...
    public function permit_update(Request $request)
    {
        ...
        try {
            ...
            if ($permitload) {
                ...
                'signature_type'=>$permitload->signature_type, 'signature_type1'=>$permitload->signature_type1]);
                $path = public_path('pdf');
