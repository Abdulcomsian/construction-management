...
$notify_admins_msg = [
    ...
    public function permit_unload_save(Request $request)
    {
        ...
        // dd($request->all());
        try {
            $all_inputs  = $request->except('_token', 'pc_email', 'twc_email', 'designer_company_email', 'companyid', 'signtype1', 'signtype', 'signed','pdfsigntype','pdfphoto','signed1', 'projno', 'projname', 'date', 'permitid', 'images', 'namesign1', 'namesign', 'design_requirement_text');
            $all_inputs['created_by'] = auth()->user()->id;
            $image_name1 = '';
            ...
            if ($permitload) {
                ...
                if ($request->principle_contractor==1) {
                    $notify_admins_msg['body']['pc_twc'] = '1';
                    Notification::route('mail', $request->pc_email ?? '')->notify(new PermitNotification($notify_admins_msg));
                } else {
                    // Notification::route('mail', 'ctwscaffolder@gmail.com')->notify(new PermitNotification($notify_admins_msg));
                    Notification::route('mail', $request->twc_email)->notify(new PermitNotification($notify_admins_msg));
                }
