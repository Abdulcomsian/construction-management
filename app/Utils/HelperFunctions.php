...
class HelperFunctions
{
    ...
    public static function isAdminDesigner($user)
    {
        ...
        if($user->di_designer_id == null || ($user->di_designer_id == null && $user->admin_designer == 1)){
            return true;
        }else{
            return false;
        }
    ...
    public static function defaultSign(){
    ...
    public static function ccEmails($ccEmails)
    {
        // Split the string into an array using the comma as the delimiter
        $ccemailArray = explode(",", $ccEmails);
        // Trim whitespace from each element and remove any empty elements
        $ccemailArray = array_map('trim', $ccemailArray);
        $ccemailArray = array_filter($ccemailArray);
        return  $ccemailArray;
    }
}
